...
2、通过字符串"a3"和md5(接口path)进行运算拼接生成20位数组，将这个数组进行魔改的RC4加密生成新的20位数组;
3、将第一步和第二步的数组拼接成144位数组，进行魔改的Base64加密生成最后的加密参数，最后在前面加上"mns0301_"字符串就完成整体的加密了;
```

- 注意逆向过程中有三个不一样的vmp，分别对应上面三个步的参数生成。这三个vmp都要解混淆插桩，不过只要写好一套ast解混淆插件模板，其他两个都能直接套用
