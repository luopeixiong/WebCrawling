...
- 京东 **h5st** 参数纯算，版本: V5.3.0
- 跟栈到加密位置，会进入一个混淆文件，这里直接就插桩看日志了（没必要解混淆，这里我解混淆发现也没啥用）。这里的vmp特别恶心，会有vm嵌套vm，导致插桩会花费很多时间，但也不是没手段解决，要不就写一个ast插件来自动化插桩，要么直接把代码扔给ai帮忙插桩。
- 这里h5st参数是通过十个参数来组成的，加密流程如下：

```
1、时间戳转换成"yyyyMMddhhmmssSSS"格式;
2、fingerprint参数，通过"ekl9i1uct6"和随机数生成;
3、appid，固定值;
4、通过request_algo请求获得，其中传参有两个localTk和expandParams加密参；localTk参数是用（fp + 时间戳 + 随机串 + 固定盐）做MD5签名，再经过hex + Base64封装，最后再做一层MD5校验拼接而成; expandParams参数是获取了浏览器的环境参数进行BASE64编码获得;
5、通过(token + fp + 时间 + appId + rd) 生成key，再用这个key对请求参数params进行HmacSHA256加密获得，这个params参数中body是进行了标准的sha256加密;
6、版本号，固定值;
7、时间戳;
8、通过浏览器的环境参数进行BASE64编码获得，与第四步的expandParams参数中获取的环境有点出入，但相差不大，加密都是一样的;
9、通过(token + fp + 时间 + appId + rd) 生成key，再用这个key对一个字符串进行HmacSHA256加密获得;
10、把参数字段名列表进行BASE64编码获得;
```

- 这里除了请求参数params中body的sha256加密是标准的，其他都是魔改的算法 。直接扣代码的里面的算法就行了，缺啥补啥。
- 请求流程：main.js

### 2026-04-04

- 小红书 **x-s** 参数纯算，版本：V4.3.3

- 跟栈跟到加密位置，简单看一下x-s参数是一个由五个参数组成的json进行字符串化，转 UTF-8 字节数组后再进行自定义字符表做Base64编码生成而来
- 再来看看这五个参数，其中里面四个参数都是固定的。重要的是x3参数，也就是mns0301参数的加密生成。往上看是通过 **window.mnsv2** 方法生成的。进去一看是动态脚本，看了一下是vmp，没什么好说的，直接写ast解混淆插件，然后本地替换运行、插桩、看日志。加密流程如下：

```
1、通过随机数、时间戳、cookie中的a1参数以及传入的参数等进行运算生成124位数组;
2、通过字符串"a3"和md5(接口path)进行运算拼接生成20位数组，将这个数组进行异或操作生成新的20位数组;
3、将第一步和第二步的数组拼接成144位数组后进行魔改的RC4加密再进行魔改的Base64加密生成最后的加密参数，最后在前面加上"mns0301_"字符串就是最后的加密值了;
```

- 注意逆向过程中有三个不一样的vmp，分别对应上面三个步骤的参数生成。这三个vmp都要解混淆插桩，不过只要写好一套ast解混淆插件模板，其他两个都能直接套用
- 看加密流程很简单对吧，实际逆向起来还真不是很难，对比抖音和美团之类的，至少它不麻烦，整体加密流程没有绕来绕去的东西。也就要弄三个vmp插桩和写插件麻烦一点
- 其实还有另一个 **x-s-common** 加密参数，这个就比较简单了，直接搜关键词就能定位到生成位置了，这个就不多说了，自己操作吧~
